...
public class ComposeNodeFilter {
     ...
     * - When used alone: matches nodes that directly contain the text
     * - When combined with composablePattern: searches subtree, so a Button
     *   containing a Text child with "Submit" will match textPattern "Submit"
     * If starts with "regex:" uses regex matching.
     */
    ...
    public String filter(String composeTreeJson) throws Exception {
        ...
        for (FilteredNode filtered : results) {
            ...
            // Include full node with children so inspector can show nested elements
            entry.set("node", filtered.getNode());
     ...
     * Text pattern searches the node AND all its descendants (subtree search).
     */
    private boolean matches(JsonNode node) {
        ...
        // Check text pattern
        // If composable filter is also set, search in subtree (find Button containing "Submit")
        // If text filter only, match the node's direct text (find nodes with text "Submit")
        if (textPattern != null && !textPattern.isEmpty()) {
            boolean hasComposableFilter = composablePattern != null && !composablePattern.isEmpty();
            if (hasComposableFilter) {
                // Subtree search: find containers (like Button) by child text
                if (!hasTextInSubtree(node, textPattern)) {
                    return false;
                }
            } else {
                // Direct match: find nodes that have this text
    ...
    /**
     * Checks if a node or any of its descendants contains text matching the pattern.
     * This enables finding containers (like Button) by the text of their children (like Text).
     */
    private boolean hasTextInSubtree(JsonNode node, String pattern) {
        if (node == null || !node.isObject()) {
            return false;
        }
        ...
        if (children != null && children.isArray()) {
            for (JsonNode child : children) {
                if (hasTextInSubtree(child, pattern)) {
                    return true;
                }
            }
        }

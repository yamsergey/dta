...
public class AppRunner {
    ...
    static String generateInitScriptContent(String variant) {
        ...
        //
        // For SNAPSHOT resolution we inject the Sonatype snapshots maven repo
        // at one of two scopes, picked at runtime per project:
        //
        //   1. settings-level via beforeSettings.dependencyResolutionManagement
        //      — always safe. Resolves snapshot artifacts for projects using
        //      RepositoriesMode.PREFER_SETTINGS or FAIL_ON_PROJECT_REPOS.
        //   2. allprojects.repositories — needed for PREFER_PROJECT (Gradle's
        //      default) where project-level repos shadow settings. But
        //      injecting at this level when the project enforces
        //      FAIL_ON_PROJECT_REPOS (e.g. Now in Android) makes Gradle
        //      hard-fail with "repository 'maven' was added by initialization
        //      script". The conditional below skips (2) in that case.
        //
    ...
    private int runGradleBuild(String projectPath, String module, String variant,
        ...
        // When the daemon is embedded in Android Studio, the AS JVM is the
        // JBR (Java 21) but the user's shell-default `java` may be newer
        // (e.g. Java 26). Gradle picks Java in this priority: the build's
        // `org.gradle.java.home`, then JAVA_HOME, then PATH `java`. If
        // none of those point at a JDK Gradle/AGP supports, we get
        // "Unsupported class file major version 70" and the build fails.
        // Set JAVA_HOME from the daemon's own JVM (System.getProperty
        // "java.home") when the inherited env doesn't already define it,
        // so the subprocess uses the same JDK that's already running this
        // process. We never overwrite a user-set JAVA_HOME — they may
        // have a deliberately older JDK on a per-project basis.
        Map<String, String> env = pb.environment();
        if (env.get("JAVA_HOME") == null || env.get("JAVA_HOME").isBlank()) {
            String javaHome = System.getProperty("java.home");
            if (javaHome != null && !javaHome.isBlank()) {
                env.put("JAVA_HOME", javaHome);
                log.info("Propagating JAVA_HOME={} to Gradle subprocess (was unset in inherited env)",
                    javaHome);
            }
        }
    ...
    public record VariantInfo(String module, String name, String applicationId) {}

    /**
     * Enumerates buildable variants for every Android application/library
     * module in the project. Runs Gradle with an init script that hooks
     * {@code androidComponents.onVariants} and emits one line per variant,
     * then parses the captured stdout.
     *
     * <p>Cost: forces full configuration of all subprojects via
     * {@code tasks --all -q}. On a configuration-cached project (e.g. NiA
     * after a recent build) this is ~5–15s; cold it's the same as a normal
     * configure pass. Caller should treat this as a one-shot per project,
     * not per-action.</p>
     */
    public List<VariantInfo> discoverVariants(String projectPath) throws IOException, InterruptedException {
        Path projectDir = Path.of(projectPath);
        String gradlew = findGradlew(projectDir);

        File initScript = File.createTempFile("dta-variant-enum-", ".gradle");
        initScript.deleteOnExit();
        // Init script hooks onVariants via plugins.withId — the AGP variant
        // API locks in variants during plugin application, so afterEvaluate
        // is too late. println (not logger.*) because -q suppresses every
        // log level except ERROR.
        Files.writeString(initScript.toPath(),
            """
            allprojects { p ->
                p.plugins.withId('com.android.application') {
                    ...
                    if (ac != null) {
                        ac.onVariants(ac.selector().all()) { v ->
                            def appId = ''
                            try { appId = v.applicationId.getOrElse('') } catch (Throwable ignored) {}
                            println("[DTA-VARIANT] ${p.path}|${v.name}|${appId}")
                        }
                    }
                }
                p.plugins.withId('com.android.library') {
                    ...
                    if (ac != null) {
                        ac.onVariants(ac.selector().all()) { v ->
                            println("[DTA-VARIANT] ${p.path}|${v.name}|")
                        }
                    }
                }
            }
        ...
        // tasks --all on the root project forces every subproject to
        // configure (it needs each one's tasks to print). --no-configuration-cache
        // because a cached config wouldn't re-run our init script.
        List<String> cmd = new ArrayList<>();
        cmd.add(gradlew);
        cmd.add("tasks");
        cmd.add("--all");
        ...
        if (env.get("JAVA_HOME") == null || env.get("JAVA_HOME").isBlank()) {
            String javaHome = System.getProperty("java.home");
            if (javaHome != null && !javaHome.isBlank()) env.put("JAVA_HOME", javaHome);
        }

        Process process = pb.start();
        List<VariantInfo> variants = new ArrayList<>();
        Set<String> seen = new HashSet<>();
        Pattern line = Pattern.compile("\\[DTA-VARIANT\\] (\\S+)\\|(\\S+)\\|(.*)");
        try (BufferedReader reader = new BufferedReader(
                new InputStreamReader(process.getInputStream(), StandardCharsets.UTF_8))) {
            String raw;
            while ((raw = reader.readLine()) != null) {
                ...
                if (m.find()) {
                    ...
                    if (seen.add(key)) {
                        String appId = m.group(3).trim();
                        variants.add(new VariantInfo(m.group(1), m.group(2), appId.isEmpty() ? null : appId));
                    }
                }
            }
        }
        ...
        if (!finished) {
            process.destroyForcibly();
            throw new IOException("Gradle variant enumeration timed out after 5 minutes");
        }
        // Sort: module path, then variant name — stable output for callers.
        variants.sort(Comparator.comparing(VariantInfo::module).thenComparing(VariantInfo::name));
        return variants;
    }

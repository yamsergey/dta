...
public class AppRunner {
    ...
    static String generateInitScriptContent(String variant) {
        ...
        //
        // When the version is a SNAPSHOT, add the Sonatype snapshots repo at
        // BOTH levels:
        //   1. settings-level via beforeSettings.dependencyResolutionManagement
        //      — required for projects using RepositoriesMode.PREFER_SETTINGS.
        //   2. allprojects.repositories — required for projects using
        //      RepositoriesMode.PREFER_PROJECT (Gradle's default) or with an
        //      explicit `allprojects { repositories { } }` in their root
        //      build.gradle that shadows settings-level repos.
        // Release versions still resolve from mavenCentral, which most
        // projects already declare; we don't pollute their repo list with
        // the snapshots URL in that case.
        boolean isSnapshot = SIDEKICK_VERSION.contains("SNAPSHOT");
        String settingsSnapshotRepo = isSnapshot
            ? "\n                        maven { url 'https://central.sonatype.com/repository/maven-snapshots/' }"
            : "";
        String projectSnapshotRepo = isSnapshot
            ? "\n                repositories {\n                    maven { url 'https://central.sonatype.com/repository/maven-snapshots/' }\n                }"
            : "";
        return """
            ...
            """.formatted(settingsSnapshotRepo, projectSnapshotRepo, SIDEKICK_VERSION, SIDEKICK_VERSION, variant);
    }

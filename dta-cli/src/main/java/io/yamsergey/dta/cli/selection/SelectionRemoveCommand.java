...
public class SelectionRemoveCommand implements Callable<Integer> {
    ...
    static class SelectionType {
        ...
        @Option(names = {"--request", "-r"}, description = "Network request ID to remove")
        String requestId;
    ...
    static class ElementSelection {
        @Option(names = {"--element", "-e"}, required = true, description = "Remove element at coordinates")
        boolean isElement;
    ...
    static class MessageSelection {
        @Option(names = {"--message", "-m"}, required = true, description = "Remove WebSocket message")
        boolean isMessage;
    ...
    public Integer call() throws Exception {
        try {
            ...
            if (selectionType.element != null) {
                ...
                result = daemon.removeSelectedElement(packageName, deviceSerial, elementJson);
            } else if (selectionType.requestId != null) {
                String json = mapper.writeValueAsString(Map.of("id", selectionType.requestId));
                result = daemon.removeSelectedNetworkRequest(packageName, deviceSerial, json);
                ...
                result = daemon.removeSelectedWebSocketMessage(packageName, deviceSerial, json);
            } else {
